...
def tracklist():
    ...
    t.reveal([
        ...
        [("   5.  ", DIM, False), ("virus       ", WHITE, True), ("Virus", FG, False)],
        [("       a threat that cannot be carried out", DIM, False)],
        [("   6.  ", DIM, False), ("mastermind  ", WHITE, True), ("Mastermind", FG, False)],
        [("       a lock, ten tries. The one that keeps score", DIM, False)],
        [("  10.  ", DIM, False), ("slipping    ", WHITE, True), ("Time Keeps On Slipping", FG, False)],
        [("       a clock that is correct for about a minute", DIM, False)],
        [("  11.  ", DIM, False), ("turbulence  ", WHITE, True), ("Turbulence", FG, False)],
        [("       a route to the tower, degrading. It reaches nothing", DIM, False)],
        [("  12.  ", DIM, False), ("upgrade     ", WHITE, True), ("Upgrade (A Brand New Day)", FG, False)],
...
def upgrade():
    ...
    t.add([None,
           ...
           None,
           [("  A brand new day.", WHITE, True)]], 3000)
    save(t, "upgrade")
